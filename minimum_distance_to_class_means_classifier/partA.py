...
# Function definition to read the input train.csv file and separate the features based on the expected class listed
# in the last column of the .csv file
def extract_data(fname):
    ...
    return data, features_class1, features_class2, class_count, feature_count


# Function definition to calculate the class mean for each feature of each class and store it in a 2D array
def class_means_calc(class_count, feature_count, features_class1, features_class2):
    class_mean = np.zeros(shape=[class_count, feature_count])
...
def class_predictor(class_mean, ip_f1, ip_f2):
    ...
    class_predicted = 1 + np.argmin([c1_norm, c2_norm])
...
def verify_test_data(fname, class_mean):
    ...
    error = sum(class_prediction != data[:, 2]) / rows
    return error
...
def classifier(fname1, fname2):
    data, features_class1, features_class2, class_count, feature_count = extract_data(fname1)
    rows, cols = np.shape(data)
    class_prediction = np.zeros([rows])
    class_mean = class_means_calc(class_count, feature_count, features_class1, features_class2)
    for count in range(0, rows):
        class_prediction[count] = class_predictor(class_mean, data[count, 0], data[count, 1])
    error1 = sum(class_prediction != data[:, 2]) / rows
    print("Filename : ", fname1)
    print("Error rate : ", error1)
    plotDecBoundaries(data[:, [0, 1]], data[:, 2], class_mean)
...
# Function call of the classifier function
classifier('synthetic1_train.csv', 'synthetic1_test.csv')
classifier('synthetic2_train.csv', 'synthetic2_test.csv')
